...
class MyOVBox(OVBox):
   def __init__(self):
      ...
      self.windowTitle = "Control min/max"
   ...
   def initialize(self):
      ...
      self.GUISliderMin = Scale(self.GUImaster, label='Min value', length=800, from_=bottom, to=top, resolution = 0.001, orient=HORIZONTAL, tickinterval=paramInterval)
      self.GUISliderMin.pack()
      self.GUISliderMax = Scale(self.GUImaster, label='Max value', length=800, from_=bottom, to=top, resolution = 0.001, orient=HORIZONTAL, tickinterval=paramInterval)
      self.GUISliderMax.pack()
      
      self.GUISliderMin.set(self.minValue)
      self.GUISliderMax.set(self.maxValue)

   def updateFromGUI(self):
     self.minValue = self.GUISliderMin.get()
     self.maxValue = self.GUISliderMax.get()
   ...
   def updateSignalBuffer(self):
      ...
      self.signalBuffer[1,:] = self.maxValue
